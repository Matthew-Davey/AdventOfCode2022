...
let countVisibleTrees i (xs : int array) =
    match i with
    | 0 ->  0
    | i when i = (Seq.length xs) - 1 -> 0
    | i ->
        let visibleLeft = xs[..i-1] |> Seq.rev |> takeWhilePrev (fun x -> x < xs[i])
        let visibleRight = xs[i+1..] |> takeWhilePrev (fun x -> x < xs[i])
...
let run (input : string array) =
    ...
    Array2D.zip h v (fun a b -> match a, b with 0, 0 -> 0 | a, b -> a * b)
    |> Array2D.toJagged
    |> Seq.map (Seq.max)
    |> Seq.max
    |> printfn "%A"
